...
- Logging and traceability for every interpretation session (Article 12)
...
SignalPath Forum transcribes speech in real time for hard-of-hearing users. It does not make decisions affecting access to services, employment, or other consequential outcomes. Users retain full agency and can request clarification if errors occur. Not listed in Annex III high-risk categories.
...
### SP-AI-004 · SignalPath CaptionLine AI
...
SignalPath CaptionLine AI provides home phone captioning for hard-of-hearing users. It does not make consequential decisions affecting natural persons. Users retain full agency. FCC captioning accuracy standards are the operative compliance framework.
...
Attrition prediction models carry real bias risk. If historical attrition data reflects structural inequity, the model will encode and amplify it. The interpreter workforce is SignalPath's operational core. Getting this wrong is not an abstract compliance problem.
...
One SignalPath-specific risk that does not exist in a standard Zendesk deployment: many Deaf users communicate in written English as a second language, with ASL as their primary language. Sentiment analysis models trained on native English writing may systematically misclassify their communications. This has to be validated before it produces a pattern of wrong escalation decisions nobody can see.
...
Autogen AI generates RFP responses reviewed and approved by humans before submission. It does not make decisions affecting natural persons. The risk shifts if proprietary pricing, FCC compliance representations, or confidential capability data is retained or used for model training by the vendor. That is a data governance concern, not an EU AI Act issue.
...
If Gong outputs inform performance reviews or termination decisions for sales staff, reclassify as High Risk under Annex III Category 4(a). That decision needs to be made explicitly, not left unresolved.
...
**Current compliance gap:** Consent disclosure consistency not confirmed. Gong's role in performance management not clarified. Final classification should not be considered confirmed until this is resolved. Priority: Medium.
...
**Current compliance gap:** Outside counsel engagement agreement almost certainly does not contain AI tool clauses. This gap is industry-wide. Most organizations have not addressed it. SignalPath can be the one that does. Priority: Critical.
...
Sentinel monitors the M365 environment for security threats. AI-automated responses (blocking accounts, quarantining files) can affect employees without immediate human review. In a VRS environment, an automated account block during a live shift takes an interpreter offline mid-call. That is both a service delivery failure and a potential FCC compliance event.
...
**Current compliance gap:** No discovery conducted. No acceptable use policy exists. No approved tool list published. This is the highest-urgency governance gap in the inventory, not because the EU AI Act tier is highest, but because it is the gap most likely to have already produced a compliance event SignalPath does not yet know about. Priority: Critical. Begin discovery immediately.
...
| SP-AI-004 SignalPath CaptionLine AI | Limited Risk | Adequate: disclosure language to confirm | Low |
...
| SP-AI-011 Gong | Limited Risk | Gap: consent consistency and performance review role unresolved | Medium |
